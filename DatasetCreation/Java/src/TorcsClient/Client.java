...
import src.TorcsClient.SimpleDriver;
import src.TorcsClient.Controller.Stage;
import src.Peer.Host;
import src.Peer.Peer;
import src.Logger.Logger;
import src.OurContinuousCharReaderUI.OurContinuousCharReaderUI;
...
public class Client 
{
	...
	*/
	...
	{
		try {
			logger = new Logger(null, null, "[Client]");
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(-1);
		}
		...
		Controller driver = load(args[0]);// new SimpleDriver();
	...
	private static Controller load(String name) {
		...
		try {
			controller = (Controller) (Object) Class.forName(name).getDeclaredConstructor().newInstance();
